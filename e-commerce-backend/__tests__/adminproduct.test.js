...
const server = setupServer(
  ...
  rest.post('http://localhost:4001/addproduct', (req, res, ctx) => {
    return res(ctx.json({ success: true, product: { ...req.body, id: 1 } }));
  }),
  rest.put('http://localhost:4001/updateproductadmin/:id', (req, res, ctx) => {
    return res(ctx.json({ success: true, product: { ...req.body, id: req.params.id } }));
  }),
  rest.delete('http://localhost:4001/removeproduct/:id', (req, res, ctx) => {
    return res(ctx.json({ success: true }));
  }),
  rest.get('http://localhost:4001/allproductsadmin', (req, res, ctx) => {
    return res(ctx.json([
